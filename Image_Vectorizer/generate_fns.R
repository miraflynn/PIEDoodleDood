...
generate_instructions <- function(edges){
  ...
  posy <- edges[1,2]
  queue <- edges
  instructions <- c(paste("GO",posx, posy, sep = ",")) # go to the first point
  ...
  while(nrow(queue) > 0){ # while there are still edges in the queue
    ...
    if(nrow(df) == 0){ # If there are no queued edges,then go to the start of the
      ...
      posx <- queue[1,1]
      posy <- queue[1,2]
      instructions <- c(instructions, paste("GO", posx, posy, sep = ","))
      instructions <- c(instructions, "DN")
      ...
      #Drive to that point
      instructions <- c(instructions, paste("GO", new_posx, new_posy, sep = ","))
